...
from stengthen import train_transform, val_test_transform
...
data_path = "D:/Assets/Study/corals_classification/Bleached Corals and Healthy Corals Classification" # todo 数据集路径
# 注： 执行之前请先划分数据集
# 超参数设置
params = {
    ...
    'early_stopping_rounds': 5,  # 早停的轮数
    'best_val_acc': 0.0,  # 用于记录最好的验证准确率
    'patience': 0,  # 用于记录已经等待的轮数
...
class SELFMODEL(nn.Module):
    def __init__(self, model_name=params['model'], out_features=params['num_classes'],
                 pretrained=True):
        super().__init__()
        self.model = timm.create_model(model_name, pretrained=False)  # 从预训练的库中加载模型（迁移学习）
        # self.model = timm.create_model(model_name, pretrained=pretrained, checkpoint_path="pretrained/resnet50d_ra2-464e36ba.pth")  # 从预训练的库中加载模型
            ...
            #self.model.load_state_dict(torch.load(checkpoint_path, map_location=params['device']))
        # classifier
        if model_name[:3] == "res":
...
if __name__ == '__main__':
    ...
    val_test_transforms = data_transforms['val']  # 验证集数据集处理方式
    train_dataset = datasets.ImageFolder(params["train_dir"], train_transforms)  # 加载训练集
    valid_dataset = datasets.ImageFolder(params["val_dir"], val_test_transforms)  # 加载验证集
    ...
    checkpoint_path = "pretrained/resnet50.pth"
    ...
    for epoch in range(1, params['epochs'] + 1):  # 开始训练
        ...
        if val_acc > params['best_val_acc']:
            ...
            params['patience'] = 0  # 重置耐心计数器
            save_path = osp.join(save_dir, f"{params['model']}_{epoch}epochs_accuracy{acc:.5f}_weights.pth")
            torch.save(model.state_dict(), save_path)
        else:
            params['patience'] += 1  # 增加耐心计数器
            # 检查是否满足早停条件
        if params['patience'] >= params['early_stopping_rounds']:
            print(f"Early stopping triggered. Best validation accuracy: {params['best_val_acc']:.4f}")
            break
        if val_acc >= best_acc:
